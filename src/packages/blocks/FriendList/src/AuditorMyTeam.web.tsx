        ...
        <Box style={{display: "flex"}}>
            ...
            <Container className="link-decoration">
                <Box className="navigation">
                    {
                        localStorage.getItem('selectUserType') == 'Auditor' ?  <Box>
                        <Typography variant="body1" >
                            {t("Buildings' Management")} / <Box component="span" style={{color: "blue"}}>{t("Team Members")}</Box>
                        </Typography>
                ...
                <Grid container spacing={3} style={{marginTop: 15, marginBottom:30}}>
                    {
                        ...
                        this.state.coreMembers.map((item:any,key:any)=> {
                            return  <TeamCard data={item.attributes} history={this.props.history} approval={false} handleDelete={(id:any) => this.handleDeleteModal(id)} openChat={this.openChat} handleEdit={(id:any) => this.handleEdit(id)}/>
...
//@ts-ignore
export default withTranslation()(withStyles(dashBoard)(withRouter(MyTeamAudi)));
...
const TeamCard = (props:any) => {
    ...
    return(
        <Grid item sm={4} md={3} xs={12} style={{position:"relative",height:"100%"}}>
           {
                localStorage.getItem('selectUserType') == 'Auditor' ?   null :   <Box style={{position:"absolute",top:"10px",right:"10px"}}>
            ...
            <Card className="EventsCards" style={{paddingLeft:"0px"}}>
                <Box style={{width:"100%",display:'flex',justifyContent:"center",alignItems:"center",flexDirection:"column",marginTop:"15px"}} onClick={() => {if(localStorage.getItem('selectUserType') !== 'Auditor')props.history.push(`/TeamMember/userDetails?id=${data.id}`)}}>
                    {
