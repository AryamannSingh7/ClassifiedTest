...
> {
  ...
  handleWithdraw = () => {
    const request = {
      "announcement": {
        "ids":[this.state.announcementID]
      }
    }
    this.withdrawAnnouncementCall(request)
  }
  ...
  withdrawAnnouncementCall = async (data:any) => {
    const societyID = localStorage.getItem("society_id")
    this.withdrawAnnouncementId = await this.apiCall({
      contentType: configJSON.contentTypeApiGetUserProfile,
      method: configJSON.callTypeApiValidateMobileNo,
      endPoint: `/society_managements/${societyID}/bx_block_announcement/announcements/withdraw_announcement`,
      body:data
    });
  }
  ...
  async receive(from: string, message: Message) {
    if(getName(MessageEnum.RestAPIResponceMessage) === message.id) {
      ...
      if(this.withdrawAnnouncementId === apiRequestCallId){
